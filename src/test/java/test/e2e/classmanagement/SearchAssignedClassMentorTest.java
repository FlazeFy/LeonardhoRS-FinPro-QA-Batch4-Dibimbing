...
public class SearchAssignedClassMentorTest extends BaseTest {
    ...
    @Test(priority = 1, groups = {"ui-test"}, description = "TC-CLMG-035 - User can view no data message when no assigned class mentor found")
    ...
    @Test(priority = 1, groups = {"ui-test"}, description = "TC-CLMG-036 - User can search assigned class mentor with valid keyword")
    public void testUserCanSearchAssignedClassMentorWithValidKeyword() {
        classPage = new ClassPage(DriverManager.getDriver());
        ...
        logger.info("TS-4: Type the mentor name");
        classPage.fillSearchClassMentor(validMentorName);
        classPage.waitForPageLoading();
