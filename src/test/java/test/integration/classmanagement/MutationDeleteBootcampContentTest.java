...
public class MutationDeleteBootcampContentTest extends BaseApiTest {
    ...
    private static final String mutation = """
     mutation deleteBootcampContent($id: String!) {
       deleteBootcampContent(id: $id) {
         id
       }
     }
    """;
    ...
    // Positive Test | P2
    @Test(priority = 4, groups = {"api-test"}, description = "TC-CLMG-026 - User can delete class content")
    public void deleteBootcampContent() {
        Map<String, Object> variables = new HashMap<>();
        variables.put("id", contentId);

        // Request
        Response response = TestUtil.templateGraphQLRequest(
                "deleteBootcampContent", mutation, variables, config.getProperty("usernameGraphQl"), config.getProperty("passwordGraphQl"), sid
        );
        JsonPath jsonPath = response.jsonPath();

        // Validate base structure
        Assert.assertNotNull(jsonPath.get("data.deleteBootcampContent"));
        Assert.assertTrue(jsonPath.get("data.deleteBootcampContent") instanceof Map);

        // Validate content props
        // Get list key / column
        Map<String, Object> dataObj = jsonPath.getMap("data.deleteBootcampContent");
